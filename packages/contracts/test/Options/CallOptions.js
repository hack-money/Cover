...
describe('CallOptions functionality', async () => {
    ...
    beforeEach(async () => {
        ...
        liquidityPool = new Contract(liquidityPoolAddress, LiquidityPool.abi, liquidityProvider);
        ...
        await paymentToken.connect(optionsBuyer).approve(optionsContract.address, numPaymentTokens);
    })
    ...
    describe('buyOption', () => {
      it("reject options resulting in zero payout to buyer", async () => {
        await expect(optionsContract.create(0, 0)).to.be.revertedWith('Amount is too small')
      })
      ...
      it("reject options below minimum duration", async () => {
        const invalidDuration = MIN_DURATION.subtract({ seconds:1 }).asSeconds();
        await expect(optionsContract.create(invalidDuration, 1000)).to.be.revertedWith('Duration is too short')
      })

      it("reject options above maximum duration", async () => {
        const invalidDuration = MAX_DURATION.add({ seconds:1 }).asSeconds();
        await expect(optionsContract.create(invalidDuration, 1000)).to.be.revertedWith('Duration is too long')
      })

      it("stores a new option", async () => {
        const amount = 100;
        const tx = await optionsContract.create(VALID_DURATION.asSeconds(), amount);
       
        // Extract optionID from emitted event.
        const receipt = await tx.wait();
        const optionID = OptionsInterface.parseLog(receipt.logs[receipt.logs.length - 1]).values.optionId;
        ...
        const expectedStart = bigNumberify(timestamp).add(ACTIVATION_DELAY.asSeconds())
        const expectedExpiration = bigNumberify(timestamp).add(VALID_DURATION.asSeconds())      
    ...
    describe('exercise', () => {
      ...
      beforeEach(async () => {
        ...
        // Extract optionID from emitted event.
        const receipt = await tx.wait();
        optionID = OptionsInterface.parseLog(receipt.logs[receipt.logs.length - 1]).values.optionId;
      })
      
      it("reject unactivated options", async () => {
        await expect(optionsContract.exercise(optionID)).to.be.revertedWith('Option has not been activated yet')
      })
      
      contextForOptionHasActivated(provider, function() {
        it("reject exercise by non-owner", async () => {
          await expect(optionsContract.connect(liquidityProvider).exercise(optionID)).to.be.revertedWith('Wrong msg.sender')
        })
      ...
      contextForOptionHasExpired(provider, function() {
        it("reject option past expiry", async () => {
          ...
          await expect(optionsContract.exercise(optionID)).to.be.revertedWith('Option has expired')
        })
      })
    })
    ...
    describe('unlock/unlockMany', () => {
      ...
      beforeEach(async () => {
        ...
        const tx1 = await optionsContract.create(VALID_DURATION.clone().subtract({days:2}).asSeconds(), amount);
        const receipt1 = await tx1.wait();
        optionID1 = OptionsInterface.parseLog(receipt1.logs[receipt1.logs.length - 1]).values.optionId

        const tx2 = await optionsContract.create(VALID_DURATION.asSeconds(), amount);
        const receipt2 = await tx2.wait();
        optionID2 = OptionsInterface.parseLog(receipt2.logs[receipt2.logs.length - 1]).values.optionId
      })
      
      contextForOptionHasActivated(provider, function() {
        it("reject unlocking unexpired option", async () => {
          await expect(optionsContract.unlock(optionID1)).to.be.revertedWith('Option has not expired yet')
        })
      })
      ...
      contextForSpecificTime(
        ...
        it("reject unlocking list of options including unexpired option", async () => {
          await expect(optionsContract.unlockMany([optionID1, optionID2])).to.be.revertedWith('Option has not expired yet')
        })
      })
